        ...
        try:
            ...
            for filepath, metadata, body in approved:
                try:
                    ...
                    self.error_handler.write_error(e, context="action_execution", extra={"file": filepath.name, "action_type": metadata.get('type', 'unknown')})
                    results['failed_count'] += 1
            ...
            self.error_handler.write_error(e, context="approval_processing_workflow", extra={"approved_count": results['approved_count']})

        # Log skill end
        self.audit.log_action(
            action_type="skill_end",
            actor="hitl_approval_handler",
            target="Approved" if results['executed_count'] > 0 else "system",
            status="completed" if results['failed_count'] == 0 else "failed",
            details={
