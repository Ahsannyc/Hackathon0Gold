...
"""
    ...
    def process_leads(self) -> Dict:
        ...
        try:
            ...
            for source_file, lead_info, body in leads:
                try:
                    ...
                    self.error_handler.write_error(e, context="lead_processing", extra={"lead": str(lead_info.get('from', 'unknown'))})
                    results['errors'] += 1

        except Exception as e:
            logger.error(f"✗ Error in process_leads: {e}")
            self.error_handler.write_error(e, context="process_leads_workflow", extra={"leads_found": results['leads_found']})
            results['errors'] += 1
        ...
        self.audit.log_action(
            ...
            actor="auto_linkedin_poster",
            target="Plans/linkedin_post_*.md" if results['posts_drafted'] > 0 else "system",
            status="completed" if results['errors'] == 0 else "failed",
            details={
                "leads_processed": results['leads_found'],
                "drafts_created": results['posts_drafted'],
