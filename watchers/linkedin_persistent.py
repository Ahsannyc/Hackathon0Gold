...
class LinkedInPersistentWatcher:
    ...
    def __init__(self):
        ...
        self.recovery = WatcherErrorRecovery("linkedin_persistent", ".")
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # Log error to /Logs/
                    self.recovery.log_error(e, context=f"cycle_{cycle_count}", retry_count=self.consecutive_failures)

                    self.consecutive_failures += 1
                    # Exponential backoff: 1s, 2s, 4s, 8s... max 60s
                    if self.recovery.should_retry(self.consecutive_failures):
                        delay = self.recovery.get_delay(self.consecutive_failures - 1)
                        logger.warning(f"[BACKOFF] Retry {self.consecutive_failures}/3 in {delay:.1f}s...")
                        time.sleep(delay)
                    else:
                        logger.error(f"[RESTART] Max retries ({self.consecutive_failures}) reached, restarting browser...")
                        break  # Exit loop, let PM2 restart
